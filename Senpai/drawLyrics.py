...
def readLyricsFile(lyricsFile = "lyrics.txt"):
    with open(lyricsFile, "r", encoding="utf8") as f:
        lyrics = f.readlines()

    return u"".join(lyrics)

def getCharacters(lyrics = readLyricsFile()):
    characters = []
    for i in range(len(lyrics)):
        if lyrics[i] not in [u"　", u"\ufeff", u"\u3000", "\n"] and lyrics[i] not in characters:
            characters.append(lyrics[i])
...
drawLyrics(readLyricsFile(), "YuGothR.ttc", loc + "\\sb\\lyrics\\")
